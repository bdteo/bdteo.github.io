...
Short version: if you are getting org dot bluez dot Error dot AuthenticationFailed with a custom C++ pairing agent on BlueZ 5.66 or newer, your internal agent registration may be the problem.
...
I spent two days staring at AuthenticationFailed before I figured out what was going on.
...
This happened while I was working on D2Explorer, a tool for pairing with the Huawei Watch D2 on Linux. The pairing error blocked everything.

Here is what happened, and how we fixed it.

## The Plan

The original plan was clean and self-contained.
...
One binary. No extra process. No external dependency.

That was the plan.

## The Wall

Everything worked up to the actual pairing call.
...
To isolate the problem, we removed the internal C++ agent from the equation.
...
The result was immediate: the same Device1 Pair call succeeded.

Consistently. Every time.

The AuthenticationFailed error vanished.

That proved the problem was not the Pair call itself. It was not the device. It was not BlueZ's ability to pair. It was specifically about how our C++ application, using sd-bus, registered and interacted as a pairing agent.
...
At first, I only had hypotheses. Later, I found evidence that this is part of a broader class of BlueZ pairing problems, not just our code.
...
For our case, the likely culprits were timing, subtle differences between sd-bus and python-dbus, and stricter BlueZ behavior in 5.66 and newer.
...
Getting past AuthenticationFailed was a big win, but it was not the end.
...
Sometimes the signals arrived. Sometimes they arrived late. Sometimes they did not arrive at all.

So we added active polling.

The pattern is simple. First, check the cached value that the signal handler updates. If that cache says the device is paired, return true. If the signal never fired, query D-Bus directly for the Paired property. If the polled value is true, update the cache and continue.
...
It is not elegant. It is necessary.

This worked too.

## The Complete Fix

If you are building automated Bluetooth pairing on Linux with BlueZ 5.66 or newer and hitting AuthenticationFailed, this is the recipe I would use.
...
If Pair still fails, check your BlueZ version and kernel version. The issue may be deeper than the pairing agent.

## What This Costs

I will not pretend this is a clean solution.

It is not.

Your app now depends on a separate Python process. Your main loop gets more complex because polling logic sits next to signal handlers. And the dream of one self-contained binary is gone.

But it works. Reliably.

And after two days staring at AuthenticationFailed, "it works" is what matters.

The original writeup links to the BlueZ issue tracker, Bleak's pairing discussion, a Red Hat kernel bug, the BlueZ agent API docs, the simple-agent.py source, and a few related Bluetooth pairing references. Those were the breadcrumbs that helped turn this from guesswork into a practical workaround.
