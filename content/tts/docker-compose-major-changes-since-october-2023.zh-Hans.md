...
[conversational tone] 最初发布于 2024 年 11 月。2026 年 3 月更新，加入 Compose v5、CVE-2025-62725、v1 移除，以及新的 spec 功能。

如果你用 Docker Compose 已经有一段时间，可能已经注意到脚下的东西在坏掉，或者悄悄变了。过去两年是 Compose 历史上变化最激进的两年，而且并不是所有变化都显而易见。

我每天都用 Compose。我的大多数开发环境都跑在它上面。东西一变，我会注意到。下面是实际重要的部分。

坏掉了什么

docker-compose 已死

[matter-of-fact] 不是 deprecated。不是维护模式。死了。 独立的 docker-compose 二进制文件，也就是基于 Python 的 v1，已经在 2025 年 4 月从 GitHub Actions runner 和官方 Docker 镜像中移除。如果你的 CI/CD pipeline 里还在引用带连字符的 docker-compose，它们已经坏了，或者马上就会坏。
...
[matter-of-fact] Watch 模式（现在生产可用）
...
[deliberate] 通过 OCI Artifacts 使用 Include
...
如果你把 include 指令和 OCI artifacts 一起使用，立刻升级到 v2.40.2 或更高版本。一个路径遍历漏洞允许攻击者在 artifact 解析期间逃出缓存目录。即使是看起来无害的 docker compose ps，如果你的 Compose 文件 include 了恶意 OCI 引用，也可能触发它。

[matter-of-fact] Docker 用 validatePathInBase() 检查修补了这个问题，但你得运行在已修复版本上。

Compose v5

Docker 从 v2 跳到了 v5（跳过 3 和 4，是为了避免和旧的文件格式版本混淆）。功能上，v5 和后期 v2 版本差不多，但它包含了一个官方 Go SDK，可用于程序化访问。这意味着你可以把 Compose 功能直接嵌入 Go 应用里，而不用 shell out 到 CLI。

检查你的版本。

[conversational tone] Bake 是默认构建工具
...
第一，从所有 Compose 文件中移除 version:。

第二，在所有脚本和 CI 配置中，把 docker-compose 替换为 docker compose。

第三，在 watch 配置中，把 x-develop 改名为 develop。

第四，如果你使用 include，升级到 v2.40.2+（CVE-2025-62725）。

[reflective] 第五，如果你不知怎么还在用 links，就把它们替换为 Docker networks。

第六，测试你的 CI -- GitHub Actions 已在 2026 年 2 月把 runner 更新到 Compose v2.40.3。
