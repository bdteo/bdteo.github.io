The Nuclear Option for Bulk Deletes: Truncate and Reinsert in MySQL and InnoDB.
...
Keep what you want, and nuke the rest.
...
Back-of-the-napkin reality check: twenty seven million rows at around six thousand rows per second is about seventy five minutes.

That is not a bug.

That is the cost model you chose.

The nuclear option: truncate and reinsert.
...
And yes, it is called nuclear for a reason. It is intentionally blunt.
...
There is no per-row delete overhead. No index updates for the rows you removed, because they are gone in one shot.
...
Do not use it when you need zero downtime.
...
Implementation in Laravel and PHP.
...
Then it does the dangerous part very explicitly.
...
That last part matters. If the script fails halfway through, you still want the connection to put the guardrails back.

Pinch of rubber-duck energy: read the helper again and ask your future self, am I sure this table can be emptied for a moment?

If the answer is not an unambiguous yes, this is not the tool.
...
If other tables reference this one, truncate may be forbidden or unsafe. Do not just disable checks and hope.
...
This is DDL plus a bulk insert. It can still cause replica lag. Do it intentionally, monitor lag, and do not pretend it is free.
...
When you are deleting almost everything, paying per deleted row is just self-inflicted pain.

Preserve what matters. Rebuild. Move on.
