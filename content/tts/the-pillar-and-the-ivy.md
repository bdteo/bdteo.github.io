The Pillar and the Ivy.

A small picture for the computer science underdog.

The textbook is not lying. It is just missing the ivy.
...
An invariant is a property P that holds at every checkpoint of an operation.

You write it down. You shrug. You go and have a coffee.

And then ten years later, you are debugging a distributed system at two A.M., and only then does the word start to mean something to you.
...
Now read the textbook line again: a property P that holds at every checkpoint of an operation.

The pillar is the property. The ivy is the operation. The checkpoint is the moment you walk past and look.

Holds is just a long way of saying: the pillar does not care about the ivy.
...
ACID. Foreign keys. Type systems. Distributed retries.
...
An idempotent operation is something you can do many times, and the result is the same as doing it once.
...
Put the two together and you have the gold standard for fault-tolerant systems.
...
A pillar that survives the ivy, and survives being hugged a thousand times.
...
But sometimes one image is the difference between a concept that lives in your bones and a concept that lives in a footnote.
...
The pillar does not care about the ivy.
