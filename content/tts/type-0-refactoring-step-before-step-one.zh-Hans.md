...
[reflective] 有一种重构，团队一直在做，通常是在压力之下，通常也不会给它命名。
...
Martin Fowler 将重构定义为在不改变外部行为的前提下改变代码的内部结构。这种精确很重要。如果行为变了，那仍然可能是有价值的工作，但严格来说它不是重构。
...
[deliberate] 普通重构可能创造更好的领域边界。Type 0 更早停下：它让现有代码说出自己已经在做什么。

这听起来很谦逊，直到你在 hotfix 中盯着一个 900 行方法，脑子开始缓冲。

丑代码里的直接问题往往不是架构。是可理解性。你无法安全改变一个你无法放进脑子里的东西。

Sonar 关于 Cognitive Complexity 的工作在这里很有用，因为它把“存在多少路径？”和“人类跟起来有多难？”分开了。Type 0 瞄准的是第二个问题。它减少 reviewer 必须在脑中模拟的状态、分支、命名歧义和视觉噪音。
...
第一，把有意义的部分提取成有名字的方法或局部变量。

第二，重新命名，让代码使用人的语言，而不是考古学。

第三，移除能够证明未被使用的噪音。

第四，围绕你即将保持的行为，添加或收紧 characterization tests。
...
[conversational tone] 如果你不小心，提取可能在这些地方改变行为：
...
[matter-of-fact] 不要因为重写后的条件“等价”就重写它。等价性就是 bugs 贴上小胡子、从安保面前走过去的地方。
...
[deliberate] 如果没有，忍住别说：“这只是 cleanup。”

这句话放出过一千个回归。

Michael Feathers 的 Working Effectively with Legacy Code 仍然是我在这里会想到的书；O'Reilly 的概览把它放在“修改 legacy systems 而不是重写一切”的框架里。实践中，有用的动作常常是一个小的 characterization test：捕捉代码当前对你即将触碰的路径实际做了什么。
...
[reflective] reviewers 一直在争论 style，因为意图不可见。
...
第三，哪个小的可读性整理，会让这两个答案在 diff 里显而易见？

[matter-of-fact] 如果第三个问题有一个小答案，就做 Type 0。
...
第一，Type 0: name existing trial eligibility checks。

第二，Fix expired subscription trial eligibility。

[deliberate] 如果 cleanup 大到让行为 diff 难以看清，就开一个单独的 PR。
...
[conversational tone] 这些可能是好主意。但它们不是 Type 0 review。
...
[matter-of-fact] 这就是整理和工程之间的区别。

Type 0 和 legacy seams

有时 Type 0 会揭示，下一个安全动作是一个 seam。

Fowler 关于 legacy seams 的笔记很有用，因为它描述了一些地方：我们可以在那里重定向、观察或测试行为，而不用在行为发生点编辑 source。在 legacy system 中，一个 seam 可能就是“我们可以测试这个”和“我们非常专业地希望它没事”之间的区别。
...
[reflective] 这可能是正确动作，但它不再只是让现有代码可理解。它改变了协作表面。把它当作 dependency-breaking refactoring，并用相应的谨慎程度 review。
...
检查项：我能说出这次 cleanup 为哪项 behavior-changing 工作做准备。

检查项：这个 PR 不会有意改变 user-visible 或 caller-visible 行为。

检查项：提取出来的方法保持 evaluation order 和 short-circuit behavior。

检查项：名字描述代码实际做的事，而不是我希望它做的事。

检查项：删除的代码已被证明在相关 runtime 中 unused，而不只是没人喜欢。

检查项：我跑了 focused tests，或者 replay 了重要场景。

检查项：如果 tests 缺失，我为 touched path 添加了 characterization coverage。

[deliberate] 检查项：PR description 告诉 reviewers 这是 Type 0，以及什么是 out of scope。

Review 期间：

检查项：先问“这是否保持行为？”再问“我是否更喜欢这个设计？”。

检查项：把 behavior changes 推到 follow-up commit 或 PR。

检查项：保留 architecture ideas 作为 notes，除非它们对 safety 是必需的。

检查项：对 clever equivalence 保持怀疑。

Merge 之后：

[reflective] 检查项：在 mental model 还新鲜的时候做真正的改动。

检查项：只有当行为有意改变时，才删除或更新 characterization tests。

检查项：不要让 Type 0 变成永恒 cleanup 的停车场。
