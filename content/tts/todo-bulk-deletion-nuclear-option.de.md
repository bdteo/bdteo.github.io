...
[matter-of-fact] Du musst Millionen von Zeilen aus einer MySQL-Tabelle löschen.
...
[flatly] Nutze das stumpfe Werkzeug nur, wenn die Form des Problems wirklich ein stumpfes Werkzeug will.
...
[deliberate] Der Prozentsatz ist keine Magie. Eine 30 Prozent-Löschung aus einer Tabelle mit furchtbaren Indexes kann trotzdem schmerzhaft sein. Eine 90 Prozent-Löschung aus einer kleinen Tabelle verdient vielleicht kein Ritual. Die eigentliche Frage ist: Welche Seite der Daten ist kleiner und sicherer zu bearbeiten?
...
Die MySQL-Dokumentation zu InnoDB-Locks lohnt sich mit Kaffee und einem kleinen Gefühl von Unbehagen: DELETE sperrt die Index-Records, die es scannt, nicht nur die Zeilen, die dein mentales Modell für Treffer hält.
...
TRUNCATE TABLE ist schnell, weil MySQL es eher wie ein Droppen und Neuerstellen der Tabelle behandelt als wie das Löschen jeder einzelnen Zeile. Die MySQL-Dokumentation zu TRUNCATE TABLE nennt die wichtigen Unterschiede: Es umgeht den normalen DML-Delete-Pfad, verursacht einen impliziten Commit, kann nicht wie ein normales DML-Statement zurückgerollt werden und feuert keine ON DELETE-Triggers.

[reflective] Also statt diesem:
...
[matter-of-fact] Wenn das um 2 Uhr morgens passiert, ist die Checkliste keine Bürokratie. Sie ist der Weg, nicht mit einem Terminal zu verhandeln.
...
Wenn du 20 Prozent einer 500GB-Tabelle behältst, ist die temporäre Kopie nicht imaginär. Sie ist ein echtes Objekt, das um echten Speicherplatz und echte I/O konkurriert.

[deadpan] 4. Binary logs und Replikas prüfen.
...
Die MySQL-Replikationsnotiz zu TRUNCATE ist kurz, aber die operative Implikation ist groß: Das ist nicht nur lokale Tabellenchirurgie.
...
MySQLs eigene Backup-Dokumentation betont Full Backups plus binary logs für Point-in-Time-Recovery. Das ist hier wichtig, weil eine schlechte Bulk-Löschung ein logischer Fehler ist, kein Plattenausfall.

Das praktische TRUNCATE + Wiedereinfügen-Runbook.

Nimm an, dass diese Tabelle kurz sicher geleert werden kann:

[deliberate] keine Child Tables hängen während der Operation von den Zeilen ab.
...
[reflective] Das ist der große Punkt. TRUNCATE, CREATE TABLE, ALTER TABLE, DROP TABLE und RENAME TABLE leben alle in MySQLs Welt impliziter Commits. Das Runbook in START TRANSACTION zu wickeln, macht es nicht sicher reversibel.
...
[matter-of-fact] Die Kopie kann länger dauern, weil Indexes auf der Keep Table existieren. Ich zahle diesen Teil der Kosten gern, wenn die Alternative ist, beim Restore zu entdecken, dass die Scratch Table nicht wie die Source geformt war.
...
[emphasized] Ich habe Foreign Keys und Triggers geprüft.
...
[deadpan] Der final row count entspricht dem keep count.
...
[reflective] Der letzte Punkt ist der Test. Wenn das Wiederherstellen der Tabelle Chaos wäre, wähle keine Operation, deren Failure Mode "Tabelle wiederherstellen" lautet.
