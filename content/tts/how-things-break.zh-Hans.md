...
[calm] 它只是一个普通的后台 job，由一个普通的用户操作触发，却把一个数据库事务撑得比需要的更宽。大多数日子里，这只是有点没礼貌。到了发布日，它就成了架构。

那条连接看起来是 idle。技术上说，是 sleeping。它没有跑 query。它不忙。它只是待在那里，仍然在迁移需要的那张表上攥着一点小小的占用。

[slows down] 睡着了，但手还搭在门把手上。
...
[deadpan] 一次发布，因为 How Things Break 而坏掉了。
...
[reflective] 教训就这么多。代码库里一种旧形状，走到了有用生命的尽头。修复并不神秘：缩小事务，强化发布路径，更新 runbook。
