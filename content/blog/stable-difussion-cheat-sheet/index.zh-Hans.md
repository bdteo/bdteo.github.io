...
title: "Stable Diffusion 速查表：故障排查与优化"
date: "2023-05-04T23:30:00.000Z"
description: "面向 SDXL、SD 3.5 和 Flux 的实用 Stable Diffusion 速查表。涵盖采样器、CFG、分辨率、负面提示词、模型选择和 UI 选择。2026 年 3 月更新。"
...
现在这是第一个决定，而且它比任何参数微调都更重要。

| 模型 | 最适合 | 分辨率 | 备注 |
|-------|----------|-----------|-------|
| **Flux 2** | 写实主义、提示词遵循度 | 1024x1024+ | 2026 年最好的开放权重写实模型。已集成进 Adobe Photoshop <small><a href="#ref1">[1]</a></small> |
| **SDXL** | 通用场景 | 1024x1024 | 微调生态极大。Juggernaut XL、Realistic Vision、DreamShaper |
| **SD 3.5 Large** | 顶级质量（Stability 的旗舰） | 1024x1024 | MMDiT 架构。SD 3.0 已于 2025 年 4 月废弃 <small><a href="#ref2">[2]</a></small> |
| **SDXL Lightning** | 速度 | 1024x1024 | 2-8 步生成。在更高分辨率下质量优于 Turbo <small><a href="#ref3">[3]</a></small> |
...
| UI | 最适合 |
|----|----------|
| **ComfyUI** | 进阶用户。基于节点，更好的显存管理，快 15%，对 Flux 支持最好。截至 2025 年，是严肃工作的行业标准 <small><a href="#ref4">[4]</a></small> |
...
| 范围 | 效果 |
|-------|--------|
| 1-4 | 非常有创造性，理解很松。经常不连贯 |
| **5-7** | 适合大多数工作的良好平衡 |
| **7-10** | 提示词遵循度强。SDXL 写实主义的甜点区 |
| 10-15 | 有伪影和颜色过度处理的风险 |
| 15+ | 几乎总是太多。伪影基本保证出现 |

**注意：** SD 3.5 使用不同的 guidance 机制。CFG 这个概念仍然适用，但尺度表现不同：从更低的值开始（3-5），再往上调。
...
超过原生分辨率会增加伪影和构图问题的风险。不要直接生成 2048x2048，用 hi-res fix 或放大。
...
- 很长的负面提示词实际上可能会*限制创造性*，并产生更差的结果
- "bad anatomy" 太模糊，没什么用。"ugly" 也不起作用，因为 SD 不是在带有 "ugly" 标签的图像上训练出来的
- 有些模型在长负面提示词下表现明显更差 <small><a href="#ref5">[5]</a></small>

**当前做法：**
1. 先不加任何负面提示词生成。
2. 如果看到具体问题（多余手指、背景模糊），再针对那个问题添加负面提示词。
3. 使用强调权重：写 `(blurry:1.3)`，而不是只写 `blurry`。
4. 保持简短：最多 5-10 个词。
...
| 8GB 卡 | 8GB | 最低可用。ComfyUI 有助于显存管理 |
...
| 显存不足 | 切换到 ComfyUI，减少 batch size，使用 fp16 |

---

### 参考资料

<a id="ref1"></a>1. [Flux 2 and NVIDIA RTX AI Integration](https://blogs.nvidia.com/blog/rtx-ai-garage-flux-2-comfyui/) -- *NVIDIA 对 Flux 2 与 ComfyUI 的介绍。*<br>
<a id="ref2"></a>2. [Stability AI Release Notes](https://platform.stability.ai/docs/release-notes) -- *SD 3.0 废弃和 3.5 发布。*<br>
<a id="ref3"></a>3. [SDXL-Lightning by ByteDance](https://huggingface.co/ByteDance/SDXL-Lightning) -- *1024px 下 2-8 步生成。*<br>
<a id="ref4"></a>4. [ComfyUI vs Automatic1111 2026 Comparison](https://wiki.shakker.ai/en/comfyui-vs-automatic1111) -- *性能和功能对比。*<br>
<a id="ref5"></a>5. [How to Use Negative Prompts Effectively](https://stable-diffusion-art.com/how-to-use-negative-prompts/) -- *关于最小负面提示词哲学的更新指南。*<br>
<a id="ref6"></a>6. [Understanding Stable Diffusion Samplers](https://civitai.com/articles/7484/understanding-stable-diffusion-samplers-beyond-image-comparisons) -- *采样器对比和选择指南。*<br>
<a id="ref7"></a>7. [Best Stable Diffusion Models for 2026](https://www.cubix.co/blog/best-model-for-stable-diffusion/) -- *当前模型格局。*

---

### 相关文章

- [Stable Diffusion 写实主义：设置与 GPU 限制指南](/zh/pushing-the-stable-diffussion-limits/) -- 深入讲解如何用当前模型实现写实结果。
