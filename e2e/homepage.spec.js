...
test.describe("Homepage - Card Rendering", () => {
  ...
  test("renders Recent Posts section with exactly 3 cards", async ({
    page,
  }) => {
  ...
  test("renders More Posts section with remaining cards", async ({ page }) => {
    const moreSection = page
      .locator(".section")
      .filter({ hasText: "More Posts" })
...
test.describe("Homepage - Scroll Reveal", () => {
  ...
  test("cards in viewport on load are visible immediately", async ({
    page,
  }) => {
  ...
  test("cards in viewport on load have no-transition class (instant reveal)", async ({
    page,
  }) => {
    await page.waitForTimeout(300)
  ...
  test("visible cards have opacity 1", async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(3, count); i++) {
      const opacity = await visibleCards
        .nth(i)
  ...
  test("scrolling reveals cards below the fold", async ({ page }) => {
    ...
    const initialVisible = await page
      .locator(".scroll-reveal.is-visible")
      .count()
    ...
    const afterScrollVisible = await page
      .locator(".scroll-reveal.is-visible")
      .count()
    expect(afterScrollVisible).toBeGreaterThan(initialVisible)
  })

  test("all cards become visible after scrolling to bottom", async ({
    page,
  }) => {
...
test.describe("Homepage - Scroll Position Restore", () => {
  test("refreshing while scrolled down does NOT show blank screen", async ({
    page,
  }) => {
    await page.goto("/")
    await page.waitForSelector(".blog-card", { timeout: 10000 })
    ...
    const visibleWithClass = await page.evaluate(
      ({ viewportHeight }) => {
        const cards = document.querySelectorAll(".scroll-reveal")
        let count = 0
        cards.forEach(card => {
  ...
  test("cards above current scroll position are also visible after restore", async ({
    page,
  }) => {
    await page.goto("/")
    await page.waitForSelector(".blog-card", { timeout: 10000 })
...
test.describe("Homepage - Card Interactions", () => {
  ...
  test("hovering dims sibling cards", async ({ page }) => {
    ...
    const siblingFilter = await cards
      .nth(1)
...
test.describe("Homepage - Card Layout", () => {
  ...
  test("card border-radius is sharp (<=4px)", async ({ page }) => {
    const card = page.locator(".blog-card").first()
    const br = await card.evaluate(el =>
  ...
  test("scroll restore works on mobile viewport", async ({ page }) => {
    ...
    const viewportCards = await page.evaluate(() => {
      ...
      document.querySelectorAll(".scroll-reveal").forEach(el => {
        ...
        if (r.top < vh && r.bottom > 0 && el.classList.contains("is-visible"))
          visible++
      })
      return visible
    })
