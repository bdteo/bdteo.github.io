...
test('debug: full link element details inside Giscus iframe', async ({ page }) => {
  // Intercept network to see what happens with CSS requests inside the iframe
  const cssResponses = [];
  page.on('response', res => {
    if (res.url().includes('/css/giscus/') || res.url().includes('dark') || res.url().includes('light')) {
      cssResponses.push({
        url: res.url(),
        status: res.status(),
        headers: res.headers(),
  ...
  page.on('requestfailed', req => {
    if (req.url().includes('/css/giscus/') || req.url().includes('dark') || req.url().includes('light')) {
      cssResponses.push({
        url: req.url(),
  ...
  const debug = await frame.evaluate(() => {
    ...
    const links = Array.from(document.querySelectorAll('link')).map(l => ({
      ...
      sheetRules: (() => {
        try { return l.sheet?.cssRules?.length ?? 'null sheet'; }
        catch (e) { return 'CORS: ' + e.message; }
      })(),
    }));
