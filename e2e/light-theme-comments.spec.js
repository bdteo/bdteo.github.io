...
test.describe("Light Theme - Text Visibility on Blog Posts", () => {
  ...
  test("article body text is visible", async ({ page }) => {
    const paragraph = page
      .locator('.blog-post [itemprop="articleBody"] p')
      .first()
  ...
  test("dump all computed colors for debugging", async ({ page }) => {
    const elements = await page.evaluate(() => {
      const selectors = [
        ...
        {
          name: "article body p",
          sel: '.blog-post [itemprop="articleBody"] p',
        },
    ...
    for (const el of elements) {
      ...
      console.log(
        `  ${el.name}: color=${el.color} bg=${el.backgroundColor} (from ${el.bgSource}) opacity=${el.opacity}`,
      )
    }
